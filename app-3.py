...
def extract_isdoc_from_binary(pdf_bytes):
    try:
...
def extract_isdoc_from_xrefs(pdf_bytes):
    try:
        with fitz.open(stream=pdf_bytes, filetype="pdf") as doc:
...
if uploaded_file:
    ...
    if uploaded_file.name.lower().endswith(".pdf"):
        ...
        if not xml_data:
            xml_data = extract_isdoc_from_binary(file_bytes)
        if not xml_data:
            xml_data = extract_isdoc_from_xrefs(file_bytes)
        if not xml_data:
